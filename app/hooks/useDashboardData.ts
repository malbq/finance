...
export function useDashboardData() {
  ...
  const data = useMemo<DashboardData | undefined>(() => {
    ...
    const balanceEvolution = generateBalanceEvolution(bankBalance, movingAverages.expectedSavings);
    ...
    const _transactions = transactions.filter(
      (tx) => tx.date.getTime() >= threeMonthsAgo.getTime(),
    );
...
function computeMovingAverages(
  ...
  for (const tx of transactions) {
    ...
    const isSalary = accountType === "BANK" && tx.type === "CREDIT" && tx.categoryId === "01010000";
...
function generateSpendingByCategory(
  ...
  for (const transaction of transactions) {
    ...
    if (excludedCategories.has(categoryId)) {
      const isSalary = accountType === "BANK" && type === "CREDIT" && categoryId === "01010000";
  ...
  return Array.from(data.values()).sort(
    (a, b) => monthKeyToIndex(a.month) - monthKeyToIndex(b.month),
  );
}
