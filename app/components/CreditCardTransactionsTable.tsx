...
export function CreditCardTransactionsTable({
  transactions,
  categories,
}: CreditCardTransactionsTableProps) {
  ...
  const formattedTransactions = useMemo(() => {
    return transactions.map((transaction) => ({
      ...
      originalPurchaseDateFormatted: transaction.originalPurchaseDate
        ? formatDate(new Date(transaction.originalPurchaseDate))
        : null,
    }))
  }, [transactions])
  ...
  const filteredTransactions = useMemo(() => {
    if (!filters.date && !filters.description && !filters.category) {
    ...
    <tr className="hover:bg-zinc-700 transition-colors">
      <td
        className={`px-4 py-2 whitespace-nowrap text-sm text-white ${
          transaction.futurePayment ? 'text-yellow-400' : ''
        }`}
      >
      ...
      <td className="px-4 py-2 whitespace-nowrap">
        <div className="text-sm text-zinc-300">
          {transaction.originalPurchaseDateFormatted || '-'}
        </div>
      </td>
      <td className="px-4 py-2 whitespace-nowrap text-sm text-zinc-300">
